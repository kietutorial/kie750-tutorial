# Modify Generated Rest Resource  

## File: MessageResource.java 
##### Path: /kiewithjhipster-02-helloworld/src/main/java/com/aikiinc/hello/web/rest/MessageResource.java
...
import com.aikiinc.hello.domain.enumeration.StatusType;
import com.aikiinc.hello.kie.service.util.KieUtils;
import com.aikiinc.hello.repository.MessageRepository;
...
//            .body(result);
		...
		try {
			...
			if (message != null) {
				// The Message fact
				message.setStatus(StatusType.HELLO);

				KieUtils.executeJbpmProcess(message, "ksession-process", "com.aikiinc.bpmn.hello");
			}
			KieUtils.showSubHeader("After  Processing Message: " + message);
...
//        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(message));
    	...
    	KieUtils.showHeader("REST request to get Message: " + id);
